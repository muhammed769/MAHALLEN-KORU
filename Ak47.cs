...
public class Ak47 : MonoBehaviour
{
   ...
   #region Deðiþkenler
    ...
    void Start()
    {
        ToplamMermiSayisi = PlayerPrefs.GetInt(Silahin_adi + "_Mermi"); // Toplam mermi sayýsýný sistemden ( Gamekontrolcu Scriptinde yazdýk. )  almýþ olduk.
        Kovan_ciksinmi = true;
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.CompareTag("Mermi")) // MermiKutusuna Mermi Tag'ýný Vermiþtik.
        {
            ...
            Mermi_Kutusu_Olustur_Yonetim.NoktalariKaldirma(other.transform.GetComponent<MermiKutusu>().Noktasi);
    ...
    void MermiAL()
    {
        ...
        if (Physics.Raycast(benimCameram.transform.position, benimCameram.transform.forward, out hit, 6))
        {
            ...
            {               
                ...
                Mermi_Kutusu_Olustur_Yonetim.NoktalariKaldirma(hit.transform.GetComponent<MermiKutusu>().Noktasi);
                   ...
                   int OlusanToplamDeger =  KalanMermi + ToplamMermiSayisi; //   // 3  // 5  + // 8 // þarjor kapasitesini aþmamasý gerekiyor bunada DÝKKAT ETMEMÝZ GEREKÝYOR.
                    ...
                    if (OlusanToplamDeger > SarjorKapasitesi)
                    {
                      ...
                      ToplamMermiSayisi =   OlusanToplamDeger - SarjorKapasitesi;
                        ...
                        PlayerPrefs.SetInt(Silahin_adi+"_Mermi", ToplamMermiSayisi); // Artýk Taramali Mermi SAyisimiz 70 den ziyade mermi kutularýný aldýktan sonra  toplam mermi sayisini hafýzada tutacaktýr artýk.
